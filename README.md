...
![Шаг 1 — выбор данных: указываем Reference Dir и Target Dir, видим сводку по эталонной папке](docs/assets/step1-data-input.png)
...
2. **Обучение профиля.** Извлекается ResNet-50 эмбеддинг + цветовая
   ...
   ![Шаг 2 — метрики валидации: confusion matrix, Acc (год/сезон), Macro F1, MAE](docs/assets/step2-train-validation-metrics.png)
...
3. **Восстановление хронологии.** Те же признаки извлекаются из
   ...
   <table><tr>
   <td><img src="docs/assets/step3-inference-timeline-chart.png" alt="Шаг 3 — гистограмма распределения по году-сезону"></td>
   <td><img src="docs/assets/step3-inference-predictions-table.png" alt="Шаг 3 — таблица предсказаний с уверенностью и путями файлов"></td>
   </tr></table>

4. **Раскладка.** Файлы копируются (или переносятся) в подпапки
   `Output Dir/YYYY-Season/` с учётом выбранного порога уверенности.

   ![Шаг 4 — настройка Output Dir, порога уверенности и операции копирования/перемещения](docs/assets/step4-export-layout.png)
