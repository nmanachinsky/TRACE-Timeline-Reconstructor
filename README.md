...
| Метрика | Цель M1 | Получено M1 | Цель M2 |
|---------|---------|-------------|---------|
| accuracy @ year | ≥ 55% | **70.2%** | ≥ 65% |
| accuracy @ (year, season) | ≥ 35% | **56.2%** | ≥ 50% |
| macro-F1 @ (year, season) | ≥ 0.25 | **0.458** | ≥ 0.40 |
| MAE, месяцев | ≤ 9 | **8.06** | ≤ 6 |

Прогон на 4486 фото (test = 1346, 28 классов). Результаты M1 уже превосходят
целевые показатели M2 по accuracy и macro-F1; M2 (faces + OCR) добавляется
как улучшение для редких классов и снижения MAE.

Подробности — `reports/metrics_m1.json`, confusion matrix —
`reports/metrics_m1_confusion.png`.
